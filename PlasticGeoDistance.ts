export default class PlasticGeoDistance {
     ...
     */
    bounds(distance: number, point: Ilatlng): Ibounds {
     ...
     */
    distance(point1: Ilatlng, point2: Ilatlng): number {
     ...
     */
    radianDistance(pA: Ilatlng, pB: Ilatlng): number {
     ...
     */
    geoToRadians(geo: Ilatlng): Ilatlng {
     ...
     */
    public geoToDegrees(geoR: Ilatlng): Ilatlng {
     ...
     */
    getBoundsCheckFunction(distance: number, point: Ilatlng): IboundsFn {
        let m: Ibounds = this.bounds(distance, point);
        return  (latlng: Ilatlng) => { return this.isInBounds(latlng, m); } ;
    }

    /**
     * is inside bounding rectangle
     */
    isInBounds(point: Ilatlng, bounds: Ibounds): boolean {
     ...
     */
    public computeMinimumDistancePoint(points: Array<Ilatlng>): IdistanceCenterResult {
        let result: IdistanceCenterResult = { point: { lat: 0, lng: 0 }, avgDist: 0, totalDist: 0 };
        ...
        let currentBest: IdistanceCenterResult = { point: midPt, avgDist: 0, totalDist: this._sumRadianDistance(midPt, pCA) };
        ...
        for (let r = 0; r < 17; r++) {
            ...
            while (currentBest.totalDist < cbd) {
                let candidatePts: Array<Ilatlngxyz> = [];
                for (let i = 0; i < 8; i++) {
     ...
     */
    private _toCenteringArray(sourceArray: Array<Ilatlng>): Array<Ilatlngxyz> {
        let result: Array<Ilatlngxyz> = [];
     ...
     * Adapted from http://www.geomidpoint.com/calculation.html - A. Geographic midpoint
     */
    private _computeAverageCoordinates(points: Array<Ilatlngxyz>): Ilatlngxyz {
        let result: Ilatlngxyz = { lat: 0, lng: 0, x: 0, y: 0, z: 0 };
        if (points && points.length > 0) {
            for (let p of points) {
                result.x += p.x;
                result.y += p.y;
     ...
     * Adapted from http://www.geomidpoint.com/calculation.html - A. Geographic midpoint
     */
    private _cartesianToRadians(cartesianPoint: Icartesian): Ilatlngxyz {
        return {
     ...
     */
    private _sumRadianDistance(fromPoint: Ilatlngxyz, toPoints: Array<Ilatlngxyz>): number {
        let result: number = 0;
     ...
     */
    private _testCandidateCenterPoints(candidates: Array<Ilatlngxyz>, points: Array<Ilatlngxyz>, existingBest: IdistanceCenterResult): IdistanceCenterResult {
        let result: IdistanceCenterResult = {
            point: {
     ...
     */
    private _destinationPointRadians(start: Ilatlng, bearing: number, distance: number): Ilatlng {
        let lat = Math.asin(Math.sin(start.lat) * Math.cos(distance) +
            Math.cos(start.lat) * Math.sin(distance) * Math.cos(bearing));
        let lng = start.lng + Math.atan2(Math.sin(bearing) * Math.sin(distance) * Math.cos(start.lat),
            Math.cos(distance) - Math.sin(start.lat) * Math.sin(lat));
        // normalize lat / lng
